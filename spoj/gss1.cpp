...
void update_sum(long long int node,long long int left,long long int right,long long int pos,long long int upd)
{
	if(left==right)
	{
		tree_sum[node] = upd;
	...
	else
	{
		long long int mid = (left+right)/2;
		if(pos<=mid)
			update_sum(2*node,left,mid,pos,upd);
		else
			update_sum(2*node + 1,mid+1,right,pos,upd);
		tree_sum[node] = tree_sum[2*node] + tree_sum[2*node+1];
	}
}
...
void update_main(long long int node,long long int left,long long int right,long long int pos,long long int val)
{
	...
	{
		tree_sum[left] = val;
		tree_best_pre[left] = val;
		tree_best_subseq[left] = val;
		tree_best_suf[left] = val;
	}
	else
	{
		long long int mid = (left+right)/2;
